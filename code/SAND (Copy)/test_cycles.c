...
int main() {
  ...
  uint64_t RoundKey_[SAND128_ROUNDS];
  ...
  KeySchedule(MasterKey, RoundKey_, SAND128_ROUNDS, 1);
  ...
  ECB_decrypt_Bitslice(CipherText, NewPlainText, RoundKey_, SAND128_ROUNDS, DATA_SIZE);
  ...
  double code_size_kB = 1.9 + 0.7;   // Code size in kB
  ...
  PrintData("Plaintext (first 64 bytes)", (uint8_t*)NewPlainText, 64);
